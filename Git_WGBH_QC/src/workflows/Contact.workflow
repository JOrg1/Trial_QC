...
<Workflow xmlns="http://soap.sforce.com/2006/04/metadata">
    <alerts>
        <fullName>Deceased_Email_Notification</fullName>
        <description>Deceased Email Notification</description>
        <protected>false</protected>
        <recipients>
            <recipient>ericka_webb@wgbh.org.qc</recipient>
            <type>user</type>
        </recipients>
        <recipients>
            <recipient>jeannette_eddy@wgbh.org.qc</recipient>
            <type>user</type>
        </recipients>
        <senderType>CurrentUser</senderType>
        <template>unfiled$public/Deceased_Notification</template>
    </alerts>
    <fieldUpdates>
...
OtherPhone 
    ...
    <fieldUpdates>
        <fullName>DeceasedCheckBox</fullName>
        <description>Updated the Deceased checkbox to True.</description>
        <field>deceased_flag__c</field>
        <literalValue>1</literalValue>
        <name>DeceasedCheckBox</name>
        <notifyAssignee>false</notifyAssignee>
    ...
    <rules>
        ...
        <active>true</active>
        <formula>AND(      ISPICKVAL( Preferred_Email__c ,&quot;Personal&quot;),      OR(           AND(                ISNEW(),                LEN(Email)&gt;0           ),           ISCHANGED( Email )      ) )</formula>
        <triggerType>onAllChanges</triggerType>
    </rules>
    <rules>
    ...
    <rules>
        ...
        <formula>OR( LEN(Email)=0, ISCHANGED(Preferred_Email__c) , ISCHANGED(WorkEmail__c) , ISCHANGED(HomeEmail__c) , ISCHANGED(Assistant_Email__c)  )</formula>
        <triggerType>onAllChanges</triggerType>
    </rules>
    <rules>
    ...
    <rules>
        <fullName>Deceased Notification</fullName>
        <actions>
            <name>Deceased_Email_Notification</name>
            <type>Alert</type>
        </actions>
        <actions>
            <name>DeceasedCheckBox</name>
        ...
        <description>Will notify the Planned Giving team when a Contact is marked as Deceased</description>
